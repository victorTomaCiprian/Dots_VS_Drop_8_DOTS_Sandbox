...
namespace DotsStencil
{
    public class DotsDebugger : IDebugger
    {
            ...
            out List<TracingStep> stepList)
        {
            ...
            foreach (var debuggingDataModel in entityFrameData.GetDebuggingSteps(currentGraphModel))
            {
                if (stepList == null)
            ...
            switch (reader.Read<DotsFrameTrace.StepType>())
            {
                ...
                case DotsFrameTrace.StepType.WrittenValue:
                    ...
                    if (!PortMapping.TryGetValue(outputDataPort.Port.Index, out portModel))
                        return false;
                    tracingStep = TracingStep.WrittenValue(portModel, writtenValue.ToString());
                    break;
                case DotsFrameTrace.StepType.ReadValue:
                    ...
                    if (!PortMapping.TryGetValue(inputDataPort.Port.Index, out portModel))
                        return false;
                    tracingStep = TracingStep.ReadValue(portModel, readValue.ToString());
                    break;
                default:
        ...
        internal void CreateDebugSymbols(Dictionary<INodeModel, GraphBuilder.MappedNode> nodeMapping, Dictionary<INodeModel, Dictionary<string, uint>> portOffsetMappings)
        {
            NodeMapping = nodeMapping.ToDictionary(p => p.Value.NodeId, p => p.Key);
            PortMapping = portOffsetMappings
                ...
                x =>
                {
                    return x.Value.Select(y =>
                    {
                        ...
                        var portModel = x.Key.GetPortModels().SingleOrDefault(p => p.UniqueId == y.Key);
                        return (PortIndex: nodeMapping[x.Key].FirstPortIndex + y.Value, PortModel: portModel);
                    });
                })
